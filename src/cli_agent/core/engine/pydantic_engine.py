...
class PydanticAgentEngine(IAgentEngine):
    ...
    def _rebuild_agent(self, user_query: Optional[str] = None):
        ...
        long_term_ctx = (
            self.long_term_memory.get_formatted_prompt_memory(user_query)
        ...
        if long_term_ctx:
            prompt_parts.append(
                f"Long-Term Memory & Project Knowledge:\n{long_term_ctx}"
            )
        ...
        def shell_execution(command: str) -> str:
            ...
            self._emit_trace(
                "tool_call", {"tool": "shell_execution", "args": {"command": command}}
            )
        ...
        def file_management(
            ...
            self._emit_trace(
                "tool_call", {"tool": "file_management", "args": clean_args}
            )
        ...
        def git_operations(
            ...
            clean_args = {k: v for k, v in args.items() if v is not None}
            self._emit_trace(
                "tool_call", {"tool": "git_operations", "args": clean_args}
            )
            res = self.skill_registry.execute(
                "git_operations",
                action=action,
