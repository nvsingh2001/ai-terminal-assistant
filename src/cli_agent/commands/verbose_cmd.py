    ...
    def execute(self, context: CommandContext, args: list[str]) -> bool:
        ...
        if new_state:
            context.console.print(
                "[bold #10b981]✓ Verbose Trace Mode: ENABLED[/bold #10b981]"
            )
            context.console.print(
                "  [dim #94a3b8]Live thinking and tool execution steps will be shown in real time.[/dim #94a3b8]\n"
            )
        else:
            context.console.print(
                "[bold #f59e0b]○ Verbose Trace Mode: DISABLED[/bold #f59e0b]"
            )
            context.console.print(
                "  [dim #94a3b8]Compact output mode restored.[/dim #94a3b8]\n"
            )
