...
from cli_agent.services import get_system_info, try_fast_path_execution
...
class NativeCLIAgent:
    ...
    def __init__(self):
        ...
        self.container = ServiceContainer.create_default(
            console=console, prompt_session=self.session
        )
        ...
        self.session.completer = WordCompleter(
            self.container.dispatcher.get_command_names()
        )
    ...
    def _handle_trace_event(self, event_type: str, data: Any):
        ...
        if event_type == "thinking":
            console.print(
                f"[bold #a78bfa]💭 Thinking:[/bold #a78bfa] [dim italic #e2e8f0]{data}[/dim italic #e2e8f0]\n"
            )
            ...
            console.print(
                f"[bold #38bdf8]⚙ Executing:[/bold #38bdf8] [bold yellow]{tool_name}[/bold yellow]([dim #94a3b8]{args_str}[/dim #94a3b8])"
            )
    ...
    def execute_request(self, user_request: str):
        ...
        if fast_res:
            ...
            console.print(
                f"[bold #a855f7]⚡ Skill Routing:[/bold #a855f7] [dim #94a3b8]{routing_str}[/dim #94a3b8]\n"
            )
        ...
        if self.verbose_enabled:
            ...
            with Live(
                Spinner(
                    "dots", text="[bold #38bdf8]Thinking & Executing...[/bold #38bdf8]"
                ),
                console=console,
                transient=True,
            ):
